...
try:
    ...
    def AppendRows(clients,mod):
        ...
        for client in clients:
            if len(client.Firstname.strip())>1:
                file.Add(client.Firstname,client.Secondname,client.Surname,client.Phone.strip())
        file.Close()
    ...
    def DeleteRow(file,phone):
        ...
        listClients = []
        for line in listFio:
            row = list(filter(lambda ph: ph.strip() == phone.strip(), line))
            if len(row)>0:
                count = count + 1
            else:
                if len(line)>1:
                    client = m.Client(line[0],line[1],line[2],line[3].strip())
                    listClients.append(client)
        if count>0:
            AppendRows(listClients,'w')
        return count

    def UpdatePhone(file,phoneOld,phoneNew):
        listFio = FileToList(file)
        row = ""
        count = 0
        listClients = []
        for line in listFio:
            row = list(filter(lambda ph: ph.strip() == phoneOld.strip(), line))
            if len(row)>0:
                count = count + 1
                client = m.Client(line[0],line[1],line[2],phoneNew)
            else:
                if len(line)>1:
                    client = m.Client(line[0],line[1],line[2],line[3].strip())
            listClients.append(client)
    ...
    print("В модуле controller какая-то проблема!")                
